...
namespace SnakeSpace
{
    ...
    void GameWorld::moveSnake(char input)
    {
        ...
        if(arrWorld[snake[0].currentpos.intx][snake[0].currentpos.inty]==FRUIT)
        {
            ...
            length=length+1;//once the fruit is eaten then increase the length of the snake
        ...
        CheckLost(snake[0].currentpos.intx,snake[0].currentpos.inty,snake[0].prevpos.intx,snake[0].prevpos.inty);
    }
    void GameWorld::CheckLost(int x,int y,int a,int b)
    {
        if(x==a && y==b)
        {
            lost=false;
    ...
    void GameWorld::IntermediateState(bool &blnInt)
    {
        blnInt=true;
    }
